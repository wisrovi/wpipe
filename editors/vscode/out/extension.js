...
var DAGPanel = class _DAGPanel {
  constructor(p, doc) {
    ...
    this._panel.webview.onDidReceiveMessage(async (message) => {
      if (message.command === "downloadSVG") {
        const saveUri = await vscode4.window.showSaveDialog({
          defaultUri: vscode4.Uri.file(message.fileName),
          filters: { "SVG Image": ["svg"] }
        });
        if (saveUri) {
          await vscode4.workspace.fs.writeFile(saveUri, new TextEncoder().encode(message.content));
          vscode4.window.showInformationMessage("\u2705 DAG exported successfully.");
        }
      }
    }, null, this._disposables);
    vscode4.workspace.onDidSaveTextDocument((e) => {
      if (e === doc) this._update(doc);
    }, null, this._disposables);
  ...
  static createOrShow(uri, doc) {
    ...
    const p = vscode4.window.createWebviewPanel("wpipeDAG", "WPipe Analysis", vscode4.ViewColumn.Two, {
      enableScripts: true,
...
${res.graph}
\`\`\``;
      await vscode4.workspace.fs.writeFile(vscode4.Uri.file(mdPath), new TextEncoder().encode(mdContent));
    } catch (e) {
    }
...
`;
    ...
    vscode10.commands.registerCommand("wpipe-vscode.runPipeline", async (filePath) => {
      ...
      const terminal = vscode10.window.createTerminal(`Pipeline: ${path4.basename(filePath)}`);
      terminal.show();
