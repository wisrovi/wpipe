...
var DAGPanel = class _DAGPanel {
    ...
    return `<html>
        ...
        <body>
            <div class="glass-header">
                <div><h1 style="margin:0;font-size:18px;font-weight:600;color:#4fc1ff;">\u{1F680} WPipe Analysis Mode</h1></div>
  ...
  parse(content2, fileName) {
    ...
    const clean = content2.replace(/#.*$/gm, "");
...
`;
      ...
      if (m[1]) {
        ...
        while ((am = addRegex.exec(clean)) !== null) {
          ...
          if (nextS !== -1 && nextS < (clean.substring(j).search(addRegex) || Infinity)) break;
        }
      }
      g += "  end\n";
    }
  ...
  parseRec(content2, prev, prefix, depth) {
    let graph = "";
    let curr = prev;
    let sCount = 0;
    let lCount = 0;
...
`;
        if (fCont) {
          const r = this.parseRec(fCont, id2, id2 + "F", depth);
...
`;
        ...
        if (pCont) {
          ...
          steps.forEach((ps, pi) => {
            const r = this.parseRec(ps, id2, id2 + "P" + pi, depth);
...
`;
        ...
        lCount++;
        graph += `  ${curr} --> ${id2}[[" \u{1F504} For Loop "]]:::logic
`;
        const fCont = this.ext(tr, "steps");
        if (fCont) {
          const r = this.parseRec(fCont, id2, id2 + "F", depth + 1);
...
`;
        ...
        if (lbl) {
          sCount++;
          graph += `  ${curr} --> ${id2}[" \u{1F680} ${lbl} "]:::step
`;
          curr = id2;
        }
      }
