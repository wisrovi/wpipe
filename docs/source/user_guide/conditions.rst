...
graph TB
    A[Start] --> B{condition}
    B -->|True| C[Branch True]
    B -->|False| D[Branch False]
    C --> E[Continue]
    D --> E
::::
...
**The condition must come AFTER a step that provides the data being evaluated.**

The condition evaluates data added by previous steps, not the initial input.
...
Only data keys and these constants are available:
- ``True``, ``False``, ``None``
...
.. code-block:: python
    ...
    Condition(expression='name.startswith("A")')
...
.. code-block:: python
    ...
    discount_condition = Condition(
        ...
        branch_false=[
            (apply_discount, "Regular Discount", "v1.0"),
        ],
    )

    no_discount_condition = Condition(
        expression="order_value < 50",
        branch_true=[(no_discount, "No Discount", "v1.0")],
    )
