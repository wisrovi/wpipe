...
::::{mermaid}
graph LR
    A["Input Data<br/>{'x': 5}"] --> B["Step 1<br/>{'y': 10}"]
    B --> C["Accumulated<br/>{'x': 5, 'y': 10}"]
    C --> D["Step 2<br/>{'z': 20}"]
    D --> E["Output<br/>{'x': 5, 'y': 10, 'z': 20}"]
::::

Key Points
~~~~~~~~~~

1. Each step receives **all accumulated data** from previous steps
2. Steps return dictionaries that are **merged** with existing data
3. Data is passed **by reference** - no explicit data passing needed
