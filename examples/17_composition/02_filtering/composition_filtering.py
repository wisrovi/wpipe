...
from wpipe.composition import NestedPipelineStep, CompositionHelper
...
def create_data_extraction_pipeline():
    ...
    pipeline.add_state("extract_raw", lambda c: {
...
def create_validation_pipeline():
    ...
    pipeline.add_state("validate_users", lambda c: {
        "valid_users": [u for u in c.get("raw_users", []) if u.get("name")]
    })
    
    pipeline.add_state("validate_posts", lambda c: {
...
def create_main_pipeline():
    ...
    nested_validate = NestedPipelineStep(
        "validate_step",
        validate_sub,
        context_filter=filter_for_validation
    )
    
    main.add_state("validate", lambda c: nested_validate.run(c))
    
    # Final aggregation
    main.add_state("finalize", lambda c: {
        "status": "complete",
