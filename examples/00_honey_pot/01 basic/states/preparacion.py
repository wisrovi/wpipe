...
@step(
    name="B_inflar_neumaticos",
    version="v1.1",
    depends_on=["A_abrir_coche"],
    parallel=True,
)
...
@step(
    name="D_arrancar_motor",
    version="v1.1",
    depends_on=["B_inflar_neumaticos", "C_limpiar_parabrisas"],
)
...
def fase_preparacion(data):
    ...
    executor.add_step(
        "B_inflar_neumaticos", B_inflar_neumaticos, mode=ExecutionMode.IO_BOUND
    )
    executor.add_step(
        "C_limpiar_parabrisas", C_limpiar_parabrisas, mode=ExecutionMode.IO_BOUND
    )
