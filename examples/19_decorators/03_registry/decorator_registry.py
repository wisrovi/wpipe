...
@step(
    name="validate_permissions", depends_on=["auth"], tags=["auth", "critical", "prod"]
)
...
@step(
    name="store_results", depends_on=["calculate_stats"], tags=["analytics", "optional"]
)
...
def run_critical_pipeline():
    ...
    critical_validate = step(
        name="validate", depends_on=["auth"], tags=["auth", "critical"]
