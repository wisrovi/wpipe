"""Simulates refueling the car to its maximum gasoline level.

This state is part of the wpipe framework and represents the action
of filling the car's fuel tank. It sets the car's gasoline level
to 'High' and returns the new gasoline level along with the car's make.

Args:
    my_car: The Car object representing the vehicle that needs refueling.

Returns:
    A dictionary containing the updated gasoline level (string) and the car's make (string).
    Example: {'gasoline_level': 'High', 'make': 'Toyota'}
"""
...
@state(name="Refuel", version="v1.0") # Renamed state name to English
@to_obj # This decorator likely converts the return dict to an object
def Refuel(my_car: Car) -> Dict[str, Any]: # Renamed function to English
    """
    Simulates refueling the car to its maximum gasoline level.
    ...
    """
    return {
        "gasoline_level": Levels.high, # Changed key to English
        "make": my_car.make,           # Changed key to English
    }

# Alias for backward compatibility or alternative naming if needed
# RefuelAlias = Refuel
