"""Simulates changing the car's engine oil.

This state is part of the wpipe framework and represents the action
of performing an oil change on a car. It sets the car's engine oil level
to 'High' and returns the updated oil level along with the car's make.

Args:
    my_car: The Car object representing the vehicle undergoing an oil change.

Returns:
    A dictionary containing the updated oil level (string) and the car's make (string).
    Example: {'oil_level': 'High', 'make': 'Toyota'}
"""
...
@state(name="ChangeOil", version="v1.0") # Renamed state name to English
@to_obj # This decorator likely converts the return dict to an object
def ChangeOil(my_car: Car) -> Dict[str, Any]: # Renamed function to English
    """
    Simulates changing the car's engine oil.
    ...
    """
    return {
        "oil_level": Levels.high, # Changed key to English
