...
import pytest
import time
from wpipe.parallel import ParallelExecutor, ExecutionMode, DAGScheduler, StepDependency
...
class TestDAGScheduler:
    ...
    def test_linear_dependencies(self):
        ...
        scheduler.add_step(StepDependency("step_2", lambda c: c, dependencies=["step_1"]))
        scheduler.add_step(StepDependency("step_3", lambda c: c, dependencies=["step_2"]))
    ...
    def test_parallel_dependencies(self):
        ...
        scheduler.add_step(StepDependency("step_1", lambda c: c, dependencies=[]))
        scheduler.add_step(StepDependency("step_2", lambda c: c, dependencies=["step_1"]))
        scheduler.add_step(StepDependency("step_3", lambda c: c, dependencies=["step_1"]))
        scheduler.add_step(StepDependency("step_4", lambda c: c, dependencies=["step_2", "step_3"]))
